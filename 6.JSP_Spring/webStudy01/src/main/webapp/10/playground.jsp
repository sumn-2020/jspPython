...
<script>
	...
	$("#crawlerForm").on("submit", function(e) {
		...
		let data = $(this).serialize(); //this에서 직렬화하면 데이터 뽑아낼수 있음 
		console.log(data);
		
		
		container.load(url, data); // 그냥 다른페이지 출력만 하는 용도라면 load를 사용해서 간단하게 만들수도 있음 (만약 다른페이지 크롤링해와서 다른 작업이 필요하다면 load말고 ajax 방식 써야됨 )
		/* $.ajax({
			url : url,
			method : method, 
			data: data,
